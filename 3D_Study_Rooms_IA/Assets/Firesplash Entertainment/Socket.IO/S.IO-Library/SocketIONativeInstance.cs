...
internal class SocketIONativeInstance : SocketIOInstance
{
    ...
    public override void Connect(string targetAddress, bool enableReconnect, SIOAuthPayload authPayload)
    {
        ...
        Task.Run(async () =>
        {
            ...
            //else if (ReconnectAttempts > 0) SIODispatcher.Instance.Enqueue(new Action(() => { RaiseSIOEvent("reconnect_attempt", ReconnectAttempts.ToString()); })); 3600
            ...
            catch (Exception e)
            {
                if (ReconnectAttempts == 0)
                {
                    ...
                    //SIODispatcher.Instance?.Enqueue(new Action(() => { RaiseSIOEvent("connect_timeout", null); })); 3948
                }
                else
                {
